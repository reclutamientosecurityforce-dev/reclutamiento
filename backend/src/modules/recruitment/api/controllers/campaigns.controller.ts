...
const campaignSchema = z.object({
  name: z.string().min(3, 'El nombre de campaña debe tener al menos 3 caracteres.'),
...
export async function getCampaigns(req: Request, res: Response): Promise<void> {
  try {
    ...
    const { rows } = await db.query<RecruitmentCampaign>(query, params);
    res.json(rows);
  } catch (err) {
...
export async function getCampaignById(req: Request, res: Response): Promise<void> {
  try {
    ...
    const { rows } = await db.query<RecruitmentCampaign>(
      `SELECT * FROM recruitment_campaigns WHERE id = $1 AND company_id = $2`,
      [id, companyId]
    );
  ...
  try {
    ...
    await db.query(
      `INSERT INTO audit_logs (company_id, user_id, action, resource, resource_id, details)
       VALUES ($1, $2, 'campaign_created', 'recruitment_campaigns', $3, $4)`,
      [companyId, userId, rows[0].id, JSON.stringify({ name: d.name, utm })]
    );
...
export async function updateCampaign(req: Request, res: Response): Promise<void> {
  try {
    ...
    await db.query(
      `INSERT INTO audit_logs (company_id, user_id, action, resource, resource_id, details)
       VALUES ($1, $2, 'campaign_updated', 'recruitment_campaigns', $3, $4)`,
...
export async function updateCampaignStatus(req: Request, res: Response): Promise<void> {
  try {
    ...
    await db.query(
      `INSERT INTO audit_logs (company_id, user_id, action, resource, resource_id, details)
       VALUES ($1, $2, 'campaign_status_changed', 'recruitment_campaigns', $3, $4)`,
      [companyId, userId, id, JSON.stringify({ status })]
    );
 ...
 */
export async function getCampaignFunnel(req: Request, res: Response): Promise<void> {
  try {
    const companyId = req.user!.companyId;
    const { id } = req.params;
    ...
    const { rows } = await db.query(query, [companyId, id]);
    res.json(rows);
  } catch (err) {
