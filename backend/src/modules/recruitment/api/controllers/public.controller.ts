...
import fs from 'fs';
import db from '../../../../db';
import { JobOpening, Candidate, Application, ApplicationDocument, StructuredProfile } from '../../../../types';
...
export async function getPublicOpenings(_req: Request, res: Response): Promise<void> {
  try {
...
export async function getPublicOpeningById(req: Request, res: Response): Promise<void> {
  try {
    const { id } = req.params;
    const { rows } = await db.query<JobOpening>(
      `SELECT 
       ...
       FROM job_openings jo
       LEFT JOIN job_categories jc ON jo.category_id = jc.id
       WHERE jo.id = $1 AND jo.status = 'open'`,
      [id]
    );
...
const initApplySchema = z.object({
...
export async function initApplication(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (d.publicationSlug) {
      const pubRes = await db.query(
        `SELECT jp.*, jo.company_id, jo.status AS opening_status, jo.title AS opening_title
         FROM job_publications jp
         JOIN job_openings jo ON jp.job_opening_id = jo.id
         WHERE jp.slug = $1`,
        [d.publicationSlug.trim().toUpperCase()]
      );
    ...
    const openingRes = await db.query<JobOpening>(
      `SELECT id, company_id, title, requirements FROM job_openings WHERE id = $1 AND status != 'cancelled'`,
      [targetOpeningId]
    );
    ...
    const result = await db.transaction(async (client) => {
      ...
      let candidateRes = await client.query<Candidate>(
        `SELECT * FROM candidates WHERE company_id = $1 AND document_number = $2`,
        [companyId, d.documentNumber]
      );
      ...
      if (candidateRes.rows.length > 0) {
        ...
        const updateRes = await client.query<Candidate>(
          `UPDATE candidates 
           SET first_name = $1, last_name = $2, phone = $3, 
               email = COALESCE($4, email), birth_date = COALESCE($5, birth_date), updated_at = NOW()
           WHERE id = $6
           RETURNING *`,
          [d.firstName, d.lastName, d.phone, d.email || null, d.birthDate || null, candidate.id]
        );
        candidate = updateRes.rows[0];
      } else {
        // Crear nuevo candidato
        const insertRes = await client.query<Candidate>(
      ...
      const appRes = await client.query<Application>(
        `SELECT * FROM applications WHERE company_id = $1 AND candidate_id = $2 AND job_opening_id = $3`,
        [companyId, candidate.id, opening.id]
      );
      ...
      if (appRes.rows.length > 0) {
        ...
        if (finalPublicationId && application.publication_id !== finalPublicationId) {
          const updAppRes = await client.query<Application>(
            `UPDATE applications
             SET publication_id = $1, channel_id = $2, campaign_id = $3,
                 utm_source = $4, utm_medium = $5, utm_campaign = $6, updated_at = NOW()
             WHERE id = $7 RETURNING *`,
            [finalPublicationId, finalChannelId, finalCampaignId, finalUtmSource, finalUtmMedium, finalUtmCampaign, application.id]
          );
        ...
        const newAppRes = await client.query<Application>(
        ...
        if (publicationRecord) {
          const activeReqsRes = await client.query(
            `SELECT * FROM opening_requirements WHERE job_opening_id = $1 AND company_id = $2 AND is_active = TRUE ORDER BY order_index ASC`,
            [opening.id, companyId]
          );

          await client.query(
            `INSERT INTO application_publication_snapshots (
...
export async function getDraftApplication(req: Request, res: Response): Promise<void> {
  try {
...
const saveProfileSchema = z.object({
  ...
  sucamecStatus: z.enum(['valid', 'in_process', 'none', 'expired']).optional(),
  sucamecCode: z.string().optional(),
  gunLicense: z.boolean().optional(),
  ...
  securityExperienceYears: z.number().optional(),
...
export async function saveDraftProfile(req: Request, res: Response): Promise<void> {
  try {
    const { draftToken } = req.params;
    ...
    const appRes = await db.query<{ id: string; candidate_id: string; job_opening_id: string }>(
      `SELECT id, candidate_id, job_opening_id FROM applications WHERE draft_token = $1`,
      [draftToken]
    );
    ...
    await db.query(
      `UPDATE applications SET progress_percentage = GREATEST(progress_percentage, 60), updated_at = NOW() WHERE id = $1`,
      [appId]
    );
...
export async function uploadCVFile(req: Request, res: Response): Promise<void> {
  try {
    const { draftToken } = req.params;
    const file = req.file;
    ...
    const appRes = await db.query<{ id: string; company_id: string; candidate_id: string }>(
      `SELECT id, company_id, candidate_id FROM applications WHERE draft_token = $1`,
      [draftToken]
    );
    ...
    // Actualizar progreso
    await db.query(
      `UPDATE applications SET progress_percentage = GREATEST(progress_percentage, 50), updated_at = NOW() WHERE id = $1`,
      [appId]
    );
...
export async function uploadDocumentPhotos(req: Request, res: Response): Promise<void> {
  try {
    const { draftToken } = req.params;
    const files = req.files as Express.Multer.File[] | undefined;
    const documentType = (req.body.documentType as string) || 'foto';
    ...
    const appRes = await db.query<{ id: string; company_id: string }>(
    ...
    const { id: appId, company_id: companyId } = appRes.rows[0];
    const storageProvider = getStorageProvider();
    const insertedDocs: ApplicationDocument[] = [];
    ...
    // Actualizar progreso
    await db.query(
      `UPDATE applications SET progress_percentage = GREATEST(progress_percentage, 75), updated_at = NOW() WHERE id = $1`,
      [appId]
    );
...
export async function deleteDraftDocument(req: Request, res: Response): Promise<void> {
  try {
    const { draftToken, docId } = req.params;

    const appRes = await db.query<{ id: string }>(
      `SELECT id FROM applications WHERE draft_token = $1`,
      [draftToken]
    );
...
export async function submitApplication(req: Request, res: Response): Promise<void> {
  try {
    ...
    const appRes = await db.query<Application & {
      ...
      candidate_security_experience_years: number;
      candidate_gun_license: boolean;
      candidate_driver_license: boolean;
    ...
    const [candRes, openRes, reqRes, docRes] = await Promise.all([
      db.query<Candidate>(`SELECT * FROM candidates WHERE id = $1`, [app.candidate_id]),
      db.query<JobOpening>(`SELECT * FROM job_openings WHERE id = $1`, [app.job_opening_id]),
      db.query<any>(`SELECT * FROM opening_requirements WHERE job_opening_id = $1 AND is_active = TRUE ORDER BY order_index ASC`, [app.job_opening_id]),
      db.query<ApplicationDocument>(`SELECT * FROM application_documents WHERE application_id = $1`, [app.id]),
    ]);

    const { EvaluationEngine } = await import('../../domain/evaluation.engine');
    ...
    // Auditoría
    await db.query(
      `INSERT INTO audit_logs (company_id, action, resource, resource_id, details, ip_address, success)
...
const trackingSchema = z.object({
  ...
  applicationCode: z.string().min(6),
});
  ...
  try {
    ...
    const { rows } = await db.query(
      `SELECT 
       ...
       WHERE c.document_number = $1 AND a.application_code ILIKE $2`,
      [documentNumber.trim(), applicationCode.trim()]
    );
...
export async function generateCVPdf(req: Request, res: Response): Promise<void> {
  try {
    const { draftToken } = req.params;
    ...
    const docsRes = await db.query<ApplicationDocument>(
      `SELECT document_type, file_name, file_path, status, uploaded_at 
    ...
    if (courses.length === 0) {
      ...
      for (const course of courses) {
        ...
        y += 12;
        doc.fontSize(8.5).font('Helvetica').fillColor('#555555').text(
          `${course.institution || ''} ${course.year ? `| Año: ${course.year}` : ''} ${course.hours ? `| ${course.hours} horas` : ''}`,
          40, y
        );
        y += 14;
      }
    ...
    doc.fontSize(8).font('Helvetica').fillColor('#888888').text(
      `Documento generado automáticamente por el Portal de Postulación Security Force P&V S.A.C. - ${new Date().toLocaleDateString('es-PE')} ${new Date().toLocaleTimeString('es-PE')}`,
      40,
      780,
...
export async function getPublicPublicationBySlug(req: Request, res: Response): Promise<void> {
  try {
    ...
    const { rows } = await db.query(query, [slug.trim().toUpperCase()]);
...
export async function registerPublicationView(req: Request, res: Response): Promise<void> {
  try {
    const { slug } = req.params;
    const { channelId, campaignId, utmSource, utmMedium, utmCampaign } = req.body;

    const pubRes = await db.query<{ id: string; company_id: string; channel_id?: string; campaign_id?: string }>(
      `SELECT id, company_id, channel_id, campaign_id FROM job_publications WHERE slug = $1`,
    ...
    const ipHash = crypto.createHash('sha256').update(`${clientIp}-${pub.id}-${todayStr}`).digest('hex');

    // 3. Verificar si es primera vista de este hash hoy
    const existingViewToday = await db.query(
      `SELECT id FROM publication_views 
       WHERE publication_id = $1 AND ip_hash = $2 AND viewed_at >= CURRENT_DATE`,
      [pub.id, ipHash]
    );
    ...
    await db.query(
      `UPDATE job_publications
       SET views_total = views_total + 1,
           ...
           updated_at = NOW()
       WHERE id = $3`,
      [isBot ? 1 : 0, isFirstViewToday ? 1 : 0, pub.id]
    );
...
export async function getDocumentRequirements(req: Request, res: Response): Promise<void> {
  try {
    ...
    const openingRes = await db.query<{ requirements: any }>(
      `SELECT requirements FROM job_openings WHERE id = $1`,
      [openingId]
    );
    ...
    const categorized = {
      IDENTIDAD: [] as any[],
      FORMACION_ACADÉMICA: [] as any[],
      EXPERIENCIA_LABORAL: [] as any[],
      SEGURIDAD: [] as any[],
      LICENCIAS: [] as any[],
      CAPACITACIÓN: [] as any[],
      OTROS: [] as any[]
    ...
    requiredDocuments.forEach((doc: any) => {
      ...
      if (categorized[category as keyof typeof categorized]) {
        categorized[category as keyof typeof categorized].push(doc);
      } else {
    ...
    const mandatory = requiredDocuments.filter((d: any) => d.required);
    const optional = requiredDocuments.filter((d: any) => !d.required);
...
export async function getExpedienteStatus(req: Request, res: Response): Promise<void> {
  try {
    ...
    const appRes = await db.query<{ id: string; job_opening_id: string }>(
      `SELECT id, job_opening_id FROM applications WHERE draft_token = $1`,
      [draftToken]
    );
    ...
    // Obtener documentos subidos
    const docsRes = await db.query<ApplicationDocument>(
      `SELECT document_type, file_name, status, uploaded_at, file_size 
       FROM application_documents 
       WHERE application_id = $1 
       ORDER BY uploaded_at ASC`,
      [appId]
    ...
    const documentStatus = requiredDocuments.map((req: any) => {
    ...
    // Calcular progreso
    const mandatoryDocs = documentStatus.filter((d: any) => d.required);
    const completedMandatory = mandatoryDocs.filter((d: any) => d.isComplete);
    ...
    const profileRes = await db.query<{ structured_profile: any }>(
      `SELECT structured_profile FROM candidates c 
    ...
    const declaredExpMonths = profile.experiences?.reduce((total: number, exp: any) => {
      if (exp.startDate) {
    ...
    res.json({
      progress: {
        ...
        optionalCompleted: documentStatus.filter((d: any) => !d.required && d.isComplete).length,
        optionalTotal: documentStatus.filter((d: any) => !d.required).length
      },
...
export async function autosaveDraft(req: Request, res: Response): Promise<void> {
  try {
    const { draftToken } = req.params;
    const {
      heightCm, weightKg, district, sucamecStatus, sucamecCode,
      gunLicense, gunLicenseType, driverLicense, driverLicenseType,
      militaryService, securityExpYears, experiences, educationList, notes,
    } = req.body;

    const appRes = await db.query<{ id: string; candidate_id: string; application_status: string }>(
      `SELECT id, candidate_id, application_status FROM applications WHERE draft_token = $1`,
      [draftToken]
    );
    ...
    if (heightCm !== undefined) { candidateUpdates.push(`height_cm = $${pIdx++}`); candidateParams.push(heightCm); }
    if (weightKg !== undefined) { candidateUpdates.push(`weight_kg = $${pIdx++}`); candidateParams.push(weightKg); }
    if (district !== undefined) { candidateUpdates.push(`district = $${pIdx++}`); candidateParams.push(district); }
    if (sucamecStatus !== undefined) { candidateUpdates.push(`sucamec_status = $${pIdx++}`); candidateParams.push(sucamecStatus); }
    if (sucamecCode !== undefined) { candidateUpdates.push(`sucamec_code = $${pIdx++}`); candidateParams.push(sucamecCode); }
    if (gunLicense !== undefined) { candidateUpdates.push(`gun_license = $${pIdx++}`); candidateParams.push(gunLicense); }
    if (gunLicenseType !== undefined) { candidateUpdates.push(`gun_license_type = $${pIdx++}`); candidateParams.push(gunLicenseType); }
    if (driverLicense !== undefined) { candidateUpdates.push(`driver_license = $${pIdx++}`); candidateParams.push(driverLicense); }
    if (driverLicenseType !== undefined) { candidateUpdates.push(`driver_license_type = $${pIdx++}`); candidateParams.push(driverLicenseType); }
    if (militaryService !== undefined) { candidateUpdates.push(`military_service = $${pIdx++}`); candidateParams.push(militaryService); }
    if (securityExpYears !== undefined) { candidateUpdates.push(`security_experience_years = $${pIdx++}`); candidateParams.push(securityExpYears); }

    // Guardar structured_profile (experiencias + educación como JSONB)
    if (experiences !== undefined || educationList !== undefined) {
      const currentProfileRes = await db.query<{ structured_profile: any }>(
        `SELECT structured_profile FROM candidates WHERE id = $1`,
        [app.candidate_id]
      );
    ...
    if (candidateUpdates.length > 0) {
      ...
      await db.query(
        `UPDATE candidates SET ${candidateUpdates.join(', ')} WHERE id = $${pIdx}`,
        candidateParams
      );
    }
    ...
    if (notes !== undefined) {
      ...
      await db.query(`UPDATE applications SET updated_at = NOW() WHERE id = $1`, [app.id]);
    }
...
export async function registerConsent(req: Request, res: Response): Promise<void> {
  try {
    const { draftToken } = req.params;
    ...
    const appRes = await db.query<{ id: string; candidate_id: string; company_id: string; application_status: string }>(
      `SELECT id, candidate_id, company_id, application_status FROM applications WHERE draft_token = $1`,
      [draftToken]
    );
    ...
    const clientIp = (req.headers['x-forwarded-for'] as string)?.split(',')[0] || req.ip || '0.0.0.0';
...
export async function getApplicationFitScore(req: Request, res: Response): Promise<void> {
  try {
    const { draftToken } = req.params;

    const appRes = await db.query<{
      id: string; candidate_id: string; job_opening_id: string; company_id: string;
      prefilter_score: number; prefilter_status: string; compatibility_score: number;
    }>(
      `SELECT id, candidate_id, job_opening_id, company_id,
              prefilter_score, prefilter_status, compatibility_score
       FROM applications WHERE draft_token = $1`,
      [draftToken]
    );
    ...
    const [reqsRes, candRes] = await Promise.all([
      db.query(
        `SELECT code, title, requirement_type, rule_type, rule_config, weight_score
         FROM opening_requirements
         WHERE job_opening_id = $1 AND company_id = $2 AND is_active = TRUE
         ORDER BY order_index ASC`,
        [application.job_opening_id, application.company_id]
      ),
      db.query(
        `SELECT height_cm, weight_kg, sucamec_status, gun_license, driver_license,
    ...
    const eliminatoriosFallidos: string[] = [];
    for (const req of requirements.filter((r: any) => r.requirement_type === 'eliminatory')) {
      const config = req.rule_config || {};
      let passes = true;
      if (req.rule_type === 'boolean') {
        const val = (candidate as any)[config.field];
        if (config.targetValue === true && !val) passes = false;
        if (config.targetValue === false && val) passes = false;
      } else if (req.rule_type === 'min_value') {
        const val = parseFloat((candidate as any)[config.field] || 0);
        if (val < config.minValue) passes = false;
      } else if (req.rule_type === 'min_months') {
        const expMo = parseInt(candidate.security_experience_years || 0) * 12;
        if (expMo < config.minMonths) passes = false;
      }
      if (!passes) eliminatoriosFallidos.push(req.title);
    }

    // Calcular puntaje puntuable
    let scorePuntuable = 0;
    let maxPuntuable = 0;
    const scoreBreakdown: Array<{ title: string; obtained: number; max: number; passed: boolean }> = [];

    for (const req of requirements.filter((r: any) => r.requirement_type === 'scoreable')) {
      const config = req.rule_config || {};
      ...
      if (req.rule_type === 'boolean') {
        const val = (candidate as any)[config.field];
        if (config.targetValue === true && val) { obtained = weight; passed = true; }
      } else if (req.rule_type === 'min_value') {
        const val = parseFloat((candidate as any)[config.field] || 0);
        if (val >= config.minValue) { obtained = weight; passed = true; }
      } else if (req.rule_type === 'min_months') {
        const expMo = parseInt(candidate.security_experience_years || 0) * 12;
        if (expMo >= config.minMonths) { obtained = weight; passed = true; }
      }
  ...
  try {
    const companyRes = await db.query<{
      ...
      `SELECT 
       ...
       ORDER BY (ruc = '20123456789') DESC, (phone IS NOT NULL AND phone != '') DESC, updated_at DESC, created_at ASC
       LIMIT 1`
    );
