...
import {
  ...
  EvaluationResult,
  PrefilterStatus,
  PrefilterScoreBreakdown,
  DiscrepancyDetail,
  WorkExperience,
} from '../../../types';
import { DocumentValidityPolicy } from './validity/document-validity.policy';
...
// ─── 1. UTILIDAD DE INTERVALOS DE FECHAS (NON-OVERLAPPING MERGE) ───────────────
 ...
 */
export function parseExperienceDate(dateStr?: string, isEnd = false): Date {
 ...
 */
export function calculateNonOverlappingMonths(experiences: WorkExperience[], positionFilter?: RegExp): number {
  if (!experiences || experiences.length === 0) return 0;

  const validIntervals: { start: Date; end: Date }[] = [];
...
export class EvaluationEngine {
   ...
   */
  public static evaluate(input: EvaluationInput): EvaluationOutput {
    ...
    const workCertDocs = documents.filter(
      (d) => d.document_type === 'cert_trabajo' || d.document_type === 'cul' || d.document_type === 'cv'
    );
    ...
    const evaluations: Omit<ApplicationEvaluation, 'id' | 'created_at'>[] = [];
    ...
    let prefilterStatus: PrefilterStatus = 'eligible';
    ...
    const prefilterBreakdown: PrefilterScoreBreakdown = {
   ...
   */
  public static calculateExpedienteScore(documents: ApplicationDocument[]): number {
    ...
    if (hasCV) score += 25;
    if (hasCUL) score += 35;
    if (hasCertTrabajo) score += 25;
    if (hasIdentityOrPhoto) score += 15;

    // Ajustes por calidad documental individual
    for (const doc of documents) {
      if (doc.status === 'vencido') score -= 25;
      else if (doc.status === 'ilegible') score -= 20;
      else if (doc.status === 'incompleto') score -= 15;
      else if (doc.status === 'observado') score -= 10;
    }
  ...
  private static evaluateSingleRequirement(params: {
    req: OpeningRequirement;
    candidate: Candidate;
    ...
    let scoreEarned = 0;
    let sourceType: 'declared' | 'extracted' | 'accredited' = 'declared';
    let declaredVal: any = null;
    let extractedVal: any = null;
    let accreditedVal: any = null;
    let evidenceDocId: string | undefined;
    let confidenceScore = 100;
    let discrepancyDetected = false;
    let discrepancyDetails: DiscrepancyDetail | undefined;
    let notes = '';
    ...
    switch (req.rule_type) {
      ...
      case 'validity': {
        ...
        const validityEval = DocumentValidityPolicy.evaluateValidity(candidate.sucamec_expires_at, {
      ...
      case 'boolean': {
        ...
        const actualVal = (candidate as any)[fieldName] === true;
