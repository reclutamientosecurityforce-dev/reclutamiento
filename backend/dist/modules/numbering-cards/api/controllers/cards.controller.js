...
exports.getCardsSummary = getCardsSummary;
exports.getCards = getCards;
exports.getCard = getCard;
exports.getCardHistory = getCardHistory;
exports.useCard = useCard;
exports.reserveCard = reserveCard;
exports.correctCard = correctCard;
exports.getMyCards = getMyCards;
exports.getRecentActivity = getRecentActivity;
exports.generateCards = generateCards;
const zod_1 = require("zod");
const db_1 = __importDefault(require("../../../../db"));
const useCardSchema = zod_1.z.object({
    observations: zod_1.z.string().max(500).optional(),
});
const correctCardSchema = zod_1.z.object({
    correctionReason: zod_1.z.string().min(1).max(500),
    newStatus: zod_1.z.enum(['available', 'cancelled']),
    observations: zod_1.z.string().max(500).optional(),
});
/**
 ...
 */
async function getCardsSummary(req, res) {
    try {
       ...
       WHERE company_id = $1`, [companyId]);
        const sessionsResult = await db_1.default.query(`SELECT COUNT(*) AS count FROM sessions
       WHERE company_id = $1 AND is_active = TRUE AND expires_at > NOW()`, [companyId]);
...
async function getCards(req, res) {
    try {
        ...
        const exactNumber = req.query.number;
        ...
        if (exactNumber && /^\d+$/.test(exactNumber)) {
         ...
         WHERE c.company_id = $1 AND c.number = $2`, [companyId, parseInt(exactNumber)]);
            res.json({
                data: rows,
                pagination: {
                    total: rows.length,
        ...
        // Contar total
        const countResult = await db_1.default.query(`SELECT COUNT(*) AS count FROM cards c WHERE ${where}`, params);
        const total = parseInt(countResult.rows[0].count);
        // Obtener página
        const { rows } = await db_1.default.query(`SELECT c.id, c.number, c.status, c.used_by, c.used_at,
...
async function getCardHistory(req, res) {
    try {
        ...
        const cardCheck = await db_1.default.query('SELECT id FROM cards WHERE id = $1 AND company_id = $2', [id, companyId]);
        if (cardCheck.rows.length === 0) {
            res.status(404).json({ error: 'Carta no encontrada.' });
            return;
        }
        const { rows } = await db_1.default.query(`SELECT ch.*, u.full_name AS user_full_name, u.username
       FROM card_history ch
       LEFT JOIN users u ON ch.user_id = u.id
       WHERE ch.card_id = $1 AND ch.company_id = $2
       ORDER BY ch.created_at DESC`, [id, companyId]);
        res.json(rows);
    }
    catch (err) {
...
async function useCard(req, res) {
    try {
        ...
        const result = await db_1.default.transaction(async (client) => {
            // SELECT FOR UPDATE — bloqueo exclusivo de la fila
            const { rows } = await client.query(`SELECT id, number, status, company_id
         FROM cards
         WHERE id = $1 AND company_id = $2
         FOR UPDATE`, [id, companyId]);
            if (rows.length === 0) {
            ...
            const { rows: updated } = await client.query(`UPDATE cards
         SET status = 'used', used_by = $1, used_at = NOW(), observations = $2, updated_at = NOW()
         WHERE id = $3 AND company_id = $4
         RETURNING *`, [userId, observations || null, id, companyId]);
            // INSERT historial
            await client.query(`INSERT INTO card_history (card_id, company_id, user_id, action, old_status, new_status, observations)
         VALUES ($1, $2, $3, 'card_used', 'available', 'used', $4)`, [id, companyId, userId, observations || null]);
            // INSERT auditoría
            await client.query(`INSERT INTO audit_logs (company_id, user_id, action, resource, resource_id, details, ip_address, user_agent, success)
 ...
 */
async function reserveCard(req, res) {
    try {
        const companyId = req.user.companyId;
        const userId = req.user.id;
        const { id } = req.params;
        ...
        const result = await db_1.default.transaction(async (client) => {
            const { rows } = await client.query('SELECT id, number, status FROM cards WHERE id = $1 AND company_id = $2 FOR UPDATE', [id, companyId]);
            if (rows.length === 0)
                throw { statusCode: 404, message: 'Carta no encontrada.' };
            const card = rows[0];
            if (card.status !== 'available') {
         ...
         WHERE id = $3 AND company_id = $4 RETURNING *`, [userId, expiresAt, id, companyId]);
            await client.query(`INSERT INTO card_history (card_id, company_id, user_id, action, old_status, new_status, metadata)
         VALUES ($1, $2, $3, 'card_reserved', 'available', 'reserved', $4)`, [id, companyId, userId, JSON.stringify({ expires_at: expiresAt })]);
            await client.query(`INSERT INTO audit_logs (company_id, user_id, action, resource, resource_id, details, ip_address, success)
         VALUES ($1, $2, 'card_reserved', 'cards', $3, $4, $5::inet, TRUE)`, [companyId, userId, id, JSON.stringify({ card_number: card.number }), req.ip]);
            return updated[0];
        });
 ...
 */
async function correctCard(req, res) {
    try {
        const companyId = req.user.companyId;
        const userId = req.user.id;
        const { id } = req.params;
        ...
        const result = await db_1.default.transaction(async (client) => {
         ...
         SET status = $1,
             ...
             updated_at = NOW()
         WHERE id = $5 AND company_id = $6 RETURNING *`, [newStatus, userId, correctionReason, observations || null, id, companyId]);
            await client.query(`INSERT INTO card_history (card_id, company_id, user_id, action, old_status, new_status, observations, metadata)
         VALUES ($1, $2, $3, 'card_corrected', $4, $5, $6, $7)`, [id, companyId, userId, oldStatus, newStatus, observations || null,
                JSON.stringify({ correction_reason: correctionReason })]);
            await client.query(`INSERT INTO audit_logs (company_id, user_id, action, resource, resource_id, details, ip_address, success)
         VALUES ($1, $2, 'card_corrected', 'cards', $3, $4, $5::inet, TRUE)`, [companyId, userId, id,
...
async function getMyCards(req, res) {
    try {
       ...
       FROM cards WHERE company_id = $2`, [userId, companyId]);
        const recentResult = await db_1.default.query(`SELECT number, status, used_at, observations
       FROM cards
       WHERE used_by = $1 AND company_id = $2
       ORDER BY used_at DESC
       LIMIT $3`, [userId, companyId, limit]);
        const rangeResult = await db_1.default.query(`SELECT name, range_start, range_end FROM numbering_ranges
...
async function generateCards(req, res) {
    try {
        ...
        const rangeCheck = await db_1.default.query('SELECT id FROM numbering_ranges WHERE id = $1 AND company_id = $2', [rangeId, companyId]);
        if (rangeCheck.rows.length === 0) {
            res.status(404).json({ error: 'Rango no encontrado.' });
            return;
        }
        const { rowCount } = await db_1.default.query(`INSERT INTO cards (company_id, range_id, number, status)
       SELECT $1, $2, n, 'available'
       FROM generate_series($3, $4) AS n
       ON CONFLICT (company_id, number) DO NOTHING`, [companyId, rangeId, rangeStart, rangeEnd]);
        await db_1.default.query(`INSERT INTO audit_logs (company_id, user_id, action, resource, details, ip_address, success)
       VALUES ($1, $2, 'cards_generated', 'cards', $3, $4::inet, TRUE)`, [companyId, userId,
