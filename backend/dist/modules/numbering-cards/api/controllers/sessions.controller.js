...
async function getSessions(req, res) {
    try {
        ...
        if (activeOnly)
            conditions.push('s.is_active = TRUE AND s.expires_at > NOW()');
        const where = conditions.join(' AND ');
        const countResult = await db_1.default.query(`SELECT COUNT(*) AS count FROM sessions s WHERE ${where}`, [companyId]);
        const { rows } = await db_1.default.query(`SELECT s.id, s.user_id, s.is_active, s.ip_address, s.user_agent,
...
async function terminateSession(req, res) {
    try {
        const companyId = req.user.companyId;
        const { id } = req.params;
        const { rowCount } = await db_1.default.query(`UPDATE sessions SET is_active = FALSE
       ...
       VALUES ($1, $2, 'session_terminated', 'sessions', $3, $4::inet, TRUE)`, [companyId, req.user.id, id, req.ip]);
