...
class CmsService {
     ...
     */
    static async getSections(companyId) {
        const knownKeys = ['hero', 'about', 'benefits', 'how_to_apply', 'footer'];
        const result = await db_1.default.query(`SELECT * FROM portal_sections
       WHERE company_id = $1
       ORDER BY section_key ASC, status ASC`, [companyId]);
     ...
     */
    static async getAdminSection(companyId, sectionKey, preferDraft = true) {
        const draftRes = await db_1.default.query(`SELECT * FROM portal_sections
       WHERE company_id = $1 AND section_key = $2 AND status = 'draft'`, [companyId, sectionKey]);
        const pubRes = await db_1.default.query(`SELECT * FROM portal_sections
       WHERE company_id = $1 AND section_key = $2 AND status = 'published'`, [companyId, sectionKey]);
     ...
     */
    static async saveDraft(companyId, sectionKey, data, userId) {
        const result = await db_1.default.query(`INSERT INTO portal_sections (
     ...
     */
    static async publishSection(companyId, sectionKey, userId) {
       ...
       WHERE company_id = $1 AND section_key = $2 AND status = 'published'`, [companyId, sectionKey]);
        ...
        let titleToPublish = null;
        let subtitleToPublish = null;
        let descToPublish = null;
        ...
        if (publishedRecord) {
            await db_1.default.query(`INSERT INTO portal_section_history (
        ...
        const publishResult = await db_1.default.query(`INSERT INTO portal_sections (
        company_id, section_key, status, version,
        title, subtitle, description, content_data, media_urls,
      ...
      RETURNING *`, [
        ...
        await db_1.default.query(`DELETE FROM portal_sections
       WHERE company_id = $1 AND section_key = $2 AND status = 'draft'`, [companyId, sectionKey]);
     ...
     */
    static async getSectionHistory(companyId, sectionKey) {
        const result = await db_1.default.query(`SELECT psh.*, u.full_name AS published_by_name, u.username AS published_by_username
       FROM portal_section_history psh
       LEFT JOIN users u ON psh.published_by = u.id
       WHERE psh.company_id = $1 AND psh.section_key = $2
       ORDER BY psh.version DESC`, [companyId, sectionKey]);
        return result.rows;
    }
    /**
     * Restaura una versión anterior del historial colocándola como nuevo borrador activo
     */
    static async restoreVersion(companyId, sectionKey, version, userId) {
        const histRes = await db_1.default.query(`SELECT * FROM portal_section_history
       WHERE company_id = $1 AND section_key = $2 AND version = $3`, [companyId, sectionKey, version]);
        ...
        const draft = await this.saveDraft(companyId, sectionKey, {
     ...
     */
    static async getPublicSection(sectionKey) {
        const fallback = this.getInstitutionalFallback(sectionKey);
        try {
            const companyRes = await db_1.default.query(`SELECT id FROM companies ORDER BY (is_active = TRUE) DESC, created_at ASC LIMIT 1`);
     ...
     */
    static async savePublicMedia(companyId, mediaData, userId) {
        const result = await db_1.default.query(`INSERT INTO portal_media (
     ...
     */
    static async getPublicMediaGallery(companyId, sectionKey) {
     ...
     */
    static getInstitutionalFallback(sectionKey) {
        const fallbacks = {
            hero: {
