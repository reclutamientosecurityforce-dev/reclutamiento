...
exports.getPipelineApplications = getPipelineApplications;
exports.advanceCandidateStage = advanceCandidateStage;
const zod_1 = require("zod");
const db_1 = __importDefault(require("../../../../db"));
const advanceStageSchema = zod_1.z.object({
    targetStage: zod_1.z.enum([
        'registered',
        'phone_screening',
        'psychological_eval',
        'background_check',
    ...
    stageScore: zod_1.z.number().min(0).max(100).optional(),
    result: zod_1.z.enum(['pending', 'passed', 'failed', 'conditional']).default('passed'),
    observations: zod_1.z.string().optional(),
    rejectionReason: zod_1.z.string().optional(),
    hiredNotes: zod_1.z.string().optional(),
});
/**
 * GET /api/pipeline
 * Lista todas las postulaciones activas agrupadas por etapa para vista Kanban o lista.
 */
async function getPipelineApplications(req, res) {
    try {
 ...
 */
async function advanceCandidateStage(req, res) {
    try {
        const companyId = req.user.companyId;
        ...
        const d = parsed.data;
        const updated = await db_1.default.transaction(async (client) => {
            // 1. Obtener la postulación
            const appRes = await client.query(`SELECT id, current_stage, candidate_id, job_opening_id 
         FROM applications 
         ...
         RETURNING *`, [d.targetStage, d.stageScore ?? null, d.rejectionReason ?? null, d.hiredNotes ?? null, id, companyId]);
            // 4. Registrar en stage_evaluations
            await client.query(`INSERT INTO stage_evaluations (
