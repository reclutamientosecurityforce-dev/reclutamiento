...
exports.getChannelsQualityMetrics = getChannelsQualityMetrics;
const zod_1 = require("zod");
const db_1 = __importDefault(require("../../../../db"));
const channelSchema = zod_1.z.object({
    name: zod_1.z.string().min(2, 'El nombre del canal debe tener al menos 2 caracteres.'),
    campaignId: zod_1.z.string().uuid().optional().nullable(),
    type: zod_1.z.enum(['facebook', 'instagram', 'whatsapp', 'qr', 'web', 'referral', 'campaign', 'other']).default('other'),
...
async function createChannel(req, res) {
    try {
        ...
        if (d.campaignId && !utmCampaign) {
            const campRes = await db_1.default.query(`SELECT utm_campaign FROM recruitment_campaigns WHERE id = $1 AND company_id = $2`, [d.campaignId, companyId]);
            if (campRes.rows.length > 0) {
                utmCampaign = campRes.rows[0].utm_campaign;
            }
        }
        const { rows } = await db_1.default.query(`INSERT INTO recruitment_channels (
       ...
       VALUES ($1, $2, 'channel_created', 'recruitment_channels', $3, $4)`, [companyId, userId, rows[0].id, JSON.stringify({ name: d.name, type: d.type })]);
...
async function updateChannel(req, res) {
    try {
        ...
        const d = parsed.data;
        const { rows } = await db_1.default.query(`UPDATE recruitment_channels
       ...
       RETURNING *`, [
        ...
        await db_1.default.query(`INSERT INTO audit_logs (company_id, user_id, action, resource, resource_id, details)
       VALUES ($1, $2, 'channel_updated', 'recruitment_channels', $3, $4)`, [companyId, userId, id, JSON.stringify({ name: rows[0].name })]);
        res.json(rows[0]);
    }
    catch (err) {
 ...
 */
async function getChannelsQualityMetrics(req, res) {
    try {
        const companyId = req.user.companyId;
        const query = `
      WITH channel_views AS (
    ...
    `;
        const { rows } = await db_1.default.query(query, [companyId]);
        res.json(rows);
    }
    catch (err) {
