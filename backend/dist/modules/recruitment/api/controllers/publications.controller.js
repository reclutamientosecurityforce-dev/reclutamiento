...
exports.duplicatePublication = duplicatePublication;
exports.deletePublication = deletePublication;
exports.getPublicationMetrics = getPublicationMetrics;
...
const publicationSchema = zod_1.z.object({
    jobOpeningId: zod_1.z.string().uuid(),
    campaignId: zod_1.z.string().uuid().optional().nullable(),
    channelId: zod_1.z.string().uuid().optional().nullable(),
    slug: zod_1.z.string().min(3).max(30).optional(),
    title: zod_1.z.string().min(5, 'El título de la publicación debe tener al menos 5 caracteres.'),
    description: zod_1.z.string().optional(),
    bannerUrl: zod_1.z.string().optional().nullable(),
    benefits: zod_1.z.array(zod_1.z.string()).default([]),
    closesAt: zod_1.z.string().optional().nullable(),
    ogTitle: zod_1.z.string().optional().nullable(),
    ogDescription: zod_1.z.string().optional().nullable(),
    ogImageUrl: zod_1.z.string().optional().nullable(),
});
/**
...
async function getPublicationById(req, res) {
    ...
    `;
        const { rows } = await db_1.default.query(query, [id, companyId]);
        if (rows.length === 0) {
            res.status(404).json({ error: 'Publicación no encontrada.' });
            return;
        }
        // Obtener los requisitos vigentes activos de la convocatoria asociada
        const reqsRes = await db_1.default.query(`SELECT * FROM opening_requirements 
       WHERE job_opening_id = $1 AND company_id = $2 AND is_active = TRUE
       ORDER BY order_index ASC`, [rows[0].job_opening_id, companyId]);
        res.json({
            publication: rows[0],
...
async function createPublication(req, res) {
    try {
        ...
        const openRes = await db_1.default.query(`SELECT id, status, title FROM job_openings WHERE id = $1 AND company_id = $2`, [d.jobOpeningId, companyId]);
        ...
        const verRes = await db_1.default.query(`SELECT COALESCE(MAX(version), 1) AS max_ver FROM opening_requirements WHERE job_opening_id = $1 AND company_id = $2`, [d.jobOpeningId, companyId]);
        ...
        let exists = await db_1.default.query(`SELECT id FROM job_publications WHERE slug = $1`, [finalSlug]);
        let attempts = 0;
        while (exists.rows.length > 0 && attempts < 5) {
            finalSlug = generateUniqueSlug();
            exists = await db_1.default.query(`SELECT id FROM job_publications WHERE slug = $1`, [finalSlug]);
            attempts++;
        }
        const { rows } = await db_1.default.query(`INSERT INTO job_publications (
       ...
       VALUES ($1, $2, 'publication_created', 'job_publications', $3, $4)`, [companyId, userId, rows[0].id, JSON.stringify({ slug: finalSlug, title: d.title })]);
...
async function updatePublication(req, res) {
    try {
        ...
        const d = parsed.data;
        const { rows } = await db_1.default.query(`UPDATE job_publications
       ...
       RETURNING *`, [
        ...
        await db_1.default.query(`INSERT INTO audit_logs (company_id, user_id, action, resource, resource_id, details)
       VALUES ($1, $2, 'publication_updated', 'job_publications', $3, $4)`, [companyId, userId, id, JSON.stringify({ title: rows[0].title })]);
        res.json(rows[0]);
    }
    catch (err) {
 ...
 */
async function changePublicationStatus(req, res) {
    try {
        const companyId = req.user.companyId;
        const userId = req.user.id;
        const { id } = req.params;
        ...
        const currentRes = await db_1.default.query(`SELECT jp.*, jo.status AS opening_status 
       FROM job_publications jp
       JOIN job_openings jo ON jp.job_opening_id = jo.id
       WHERE jp.id = $1 AND jp.company_id = $2`, [id, companyId]);
       ...
       RETURNING *`, [targetStatus, id, companyId]);
        await db_1.default.query(`INSERT INTO audit_logs (company_id, user_id, action, resource, resource_id, details)
       VALUES ($1, $2, 'publication_status_changed', 'job_publications', $3, $4)`, [companyId, userId, id, JSON.stringify({ from: currentStatus, to: targetStatus })]);
        res.json(rows[0]);
    }
    catch (err) {
 ...
 */
async function duplicatePublication(req, res) {
    try {
        const companyId = req.user.companyId;
        const userId = req.user.id;
        const { id } = req.params;
        const { targetJobOpeningId, newTitle } = req.body;
        const origRes = await db_1.default.query(`SELECT * FROM job_publications WHERE id = $1 AND company_id = $2`, [id, companyId]);
        ...
        const verRes = await db_1.default.query(`SELECT COALESCE(MAX(version), 1) AS max_ver FROM opening_requirements WHERE job_opening_id = $1 AND company_id = $2`, [finalOpeningId, companyId]);
        const reqVersion = verRes.rows[0].max_ver;
        const { rows } = await db_1.default.query(`INSERT INTO job_publications (
         company_id, job_opening_id, campaign_id, channel_id, slug,
         title, description, banner_url, benefits, requirement_version_at_publish,
       ...
       RETURNING *`, [
        ...
        await db_1.default.query(`INSERT INTO audit_logs (company_id, user_id, action, resource, resource_id, details)
       VALUES ($1, $2, 'publication_duplicated', 'job_publications', $3, $4)`, [companyId, userId, rows[0].id, JSON.stringify({ originalId: id, newSlug })]);
        res.status(201).json(rows[0]);
    }
    catch (err) {
...
async function deletePublication(req, res) {
    try {
        ...
        const appsCountRes = await db_1.default.query(`SELECT COUNT(*) AS count FROM applications WHERE publication_id = $1 AND company_id = $2`, [id, companyId]);
        ...
        const { rows } = await db_1.default.query(`DELETE FROM job_publications WHERE id = $1 AND company_id = $2 RETURNING *`, [id, companyId]);
        if (rows.length === 0) {
            res.status(404).json({ error: 'Publicación no encontrada.' });
            return;
        }
        await db_1.default.query(`INSERT INTO audit_logs (company_id, user_id, action, resource, resource_id, details)
       VALUES ($1, $2, 'publication_deleted', 'job_publications', $3, $4)`, [companyId, userId, id, JSON.stringify({ slug: rows[0].slug })]);
 ...
 */
async function getPublicationMetrics(req, res) {
    try {
        const companyId = req.user.companyId;
        const { id } = req.params;
      ...
      WHERE jp.id = $1 AND jp.company_id = $2
    `;
        const summaryRes = await db_1.default.query(summaryQuery, [id, companyId]);
    ...
    `;
        const channelsRes = await db_1.default.query(channelsQuery, [id, companyId]);
      ...
      ORDER BY d.day ASC
    `;
        const timelineRes = await db_1.default.query(timelineQuery, [id]);
        res.json({
