...
exports.getPublicOpeningDocumentRequirements = getPublicOpeningDocumentRequirements;
exports.getPublicExpedienteStatus = getPublicExpedienteStatus;
exports.uploadDocumentWithClassification = uploadDocumentWithClassification;
exports.confirmDocumentClassification = confirmDocumentClassification;
exports.getApplicationDocuments = getApplicationDocuments;
exports.deleteApplicationDocument = deleteApplicationDocument;
 ...
 */
async function getPublicOpeningDocumentRequirements(req, res) {
    try {
        ...
        const requirements = await document_requirements_service_1.DocumentRequirementsService.getOpeningDocumentRequirements(openingId, companyId && companyId !== 'default' ? companyId : undefined);
 ...
 */
async function getPublicExpedienteStatus(req, res) {
    try {
        const { draftToken } = req.params;
        // Obtener application_id desde draft_token
        const appResult = await db_1.default.query(`SELECT id, company_id FROM applications WHERE draft_token = $1`, [draftToken]);
 ...
 */
async function uploadDocumentWithClassification(req, res) {
    try {
        const { draftToken } = req.params;
        ...
        const docResult = await db_1.default.query(`INSERT INTO application_documents (
      ...
      RETURNING *`, [
        ...
        if (classification.method === 'ai' && classification.confidence > 0.7) {
            await db_1.default.query(`INSERT INTO document_classifications (
 ...
 */
async function confirmDocumentClassification(req, res) {
    try {
        ...
        const appResult = await db_1.default.query(`SELECT ad.id, ad.company_id 
       FROM application_documents ad
       JOIN applications a ON ad.application_id = a.id
       WHERE ad.id = $1 AND a.draft_token = $2`, [documentId, draftToken]);
        if (appResult.rows.length === 0) {
       ...
       WHERE id = $3`, [confirmedType, confirmedCategory, id]);
      ...
      DO UPDATE SET 
        ...
        confirmed_at = NOW(),
        classification_method = 'user'`, [companyId, id, confirmedType, confirmedCategory]);
...
async function getApplicationDocuments(req, res) {
    try {
        const { draftToken } = req.params;
        const result = await db_1.default.query(`SELECT ad.*, odr.title as requirement_title, odr.is_required
       FROM application_documents ad
       LEFT JOIN opening_document_requirements odr ON ad.requirement_id = odr.id
       JOIN applications a ON ad.application_id = a.id
       WHERE a.draft_token = $1
       ORDER BY ad.uploaded_at DESC`, [draftToken]);
...
async function deleteApplicationDocument(req, res) {
    try {
        const { draftToken, documentId } = req.params;
        // Verificar que el documento pertenece a la postulación
        const appResult = await db_1.default.query(`SELECT ad.id, ad.storage_key, ad.company_id 
       FROM application_documents ad
       JOIN applications a ON ad.application_id = a.id
       WHERE ad.id = $1 AND a.draft_token = $2`, [documentId, draftToken]);
        if (appResult.rows.length === 0) {
        ...
        await db_1.default.query(`DELETE FROM application_documents WHERE id = $1 AND company_id = $2`, [id, companyId]);
        res.status(204).send();
    }
    catch (err) {
 ...
 */
async function classifyDocument(file, providedType, providedCategory) {
