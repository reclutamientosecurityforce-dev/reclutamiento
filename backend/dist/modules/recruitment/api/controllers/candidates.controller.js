...
exports.getCandidateExpediente = getCandidateExpediente;
...
const candidateSchema = zod_1.z.object({
    ...
    city: zod_1.z.string().default('Lima'),
    birthDate: zod_1.z.string().optional(),
    gender: zod_1.z.string().optional(),
    heightCm: zod_1.z.number().int().positive().optional(),
    weightKg: zod_1.z.number().positive().optional(),
    sucamecStatus: zod_1.z.enum(['valid', 'in_process', 'none', 'expired']).default('none'),
    sucamecCode: zod_1.z.string().optional(),
    sucamecExpiresAt: zod_1.z.string().optional(),
    gunLicense: zod_1.z.boolean().default(false),
    gunLicenseType: zod_1.z.string().optional(),
    driverLicense: zod_1.z.boolean().default(false),
    driverLicenseType: zod_1.z.string().optional(),
    militaryService: zod_1.z.boolean().default(false),
    securityExperienceYears: zod_1.z.number().int().nonnegative().default(0),
    notes: zod_1.z.string().optional(),
    jobOpeningId: zod_1.z.string().uuid().optional(), // postulación inicial opcional
});
/**
 * GET /api/candidates
...
async function getCandidates(req, res) {
    try {
        ...
        const sucamec = req.query.sucamec;
        const gunLicense = req.query.gunLicense;
        ...
        const countRes = await db_1.default.query(`SELECT COUNT(*) AS count FROM candidates c WHERE ${where}`, params);
...
async function getCandidate(req, res) {
    try {
        const companyId = req.user.companyId;
        const { id } = req.params;
        const { rows } = await db_1.default.query(`SELECT c.* FROM candidates c WHERE c.id = $1 AND c.company_id = $2`, [id, companyId]);
        if (rows.length === 0) {
            res.status(404).json({ error: 'Postulante no encontrado.' });
            return;
        }
        // Obtener historial de postulaciones y etapas
        const appsRes = await db_1.default.query(`SELECT 
       ...
       ORDER BY a.created_at DESC`, [id, companyId]);
        res.json({
            ...rows[0],
...
async function createCandidate(req, res) {
    try {
        ...
        const candidate = await db_1.default.transaction(async (client) => {
            const { rows } = await client.query(`INSERT INTO candidates (
           ...
           $22, $23, $24, $25
            ...
            if (d.jobOpeningId) {
                await client.query(`INSERT INTO applications (company_id, candidate_id, job_opening_id, current_stage, assigned_recruiter)
           VALUES ($1, $2, $3, 'registered', $4)`, [companyId, newCand.id, d.jobOpeningId, userId]);
            }
...
async function updateCandidate(req, res) {
    try {
        ...
        const d = parsed.data;
        const { rows } = await db_1.default.query(`UPDATE candidates
 ...
 */
async function getCandidateExpediente(req, res) {
    try {
        const companyId = req.user.companyId;
        const { id } = req.params;
        // 1. Datos del candidato
        const candRes = await db_1.default.query(`SELECT * FROM candidates WHERE id = $1 AND company_id = $2`, [id, companyId]);
       ...
       ORDER BY a.created_at DESC`, [id, companyId]);
        ...
        let docs = [];
        if (appIds.length > 0) {
            const docsRes = await db_1.default.query(`SELECT * FROM application_documents WHERE application_id = ANY($1) ORDER BY uploaded_at DESC`, [appIds]);
            docs = docsRes.rows;
        }
        // 4. Entrevistas registradas
        let interviews = [];
        if (appIds.length > 0) {
            const intRes = await db_1.default.query(`SELECT i.*, u.full_name AS interviewer_name
         FROM interviews i
         LEFT JOIN users u ON i.interviewer_id = u.id
         WHERE i.application_id = ANY($1)
         ORDER BY i.interview_date DESC`, [appIds]);
        ...
        const nonOverlappingMonths = (0, evaluation_engine_1.calculateNonOverlappingMonths)(experiences);
        const declaredYears = Number(candidate.security_experience_years) || 0;
        const declaredMonths = declaredYears * 12;
        const hasWorkCerts = docs.some((d) => d.document_type === 'cert_trabajo' || d.document_type === 'cul');
        ...
        const rawMonthsSum = experiences.reduce((acc, exp) => {
            if (!exp.startDate)
                return acc;
